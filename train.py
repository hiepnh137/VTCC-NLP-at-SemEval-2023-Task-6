    ...
    def run_training_for_fold(self, fold_num, fold: Fold, initial_model=None, return_best_model=False):
        ...
        while epoch < max_train_epochs and early_stopping_counter < early_stopping:
            ...
            for batch_num, batch in enumerate(train_batches):
                ...
                batch['graph'][0] = batch['graph'][0].to(self.device)
#                 print(batch)
                documents, sentences, tokens = batch["input_ids"].shape
                output, lsp_output, constrastive_output = model(
                    batch=batch,
                    labels=batch["label_ids"]
                )
                loss = self.config['crf_loss_coeff'] * output["loss"] + \
                    self.config['lsp_loss_coeff'] * lsp_output["loss"] + \
                    self.config['constrastive_loss_coeff'] * constrastive_output['loss']
                loss = loss.sum()
                loss.backward()
            ...
            dev_metrics, dev_confusion, labels_dict, _, dev_lsp_metrics, dev_lsp_confusion, dev_lsp_class_report = eval_model(model, dev_batches, self.device, self.task)
            ...
            if dev_metrics[self.task.dev_metric] > best_dev_result:
                ...
                results={}
                test_metrics, test_confusion, labels_dict, _, test_lsp_metrics, test_lsp_confusion, test_lsp_class_report= eval_model(model, test_batches, self.device, self.task)
                results['dev_metrics']=dev_metrics
                results['dev_confusion'] = dev_confusion
                results['labels_dict'] = labels_dict
                results['classification_report']=_
                ...
                self.result_writer.log(
                    f'*** fold: {fold_num},  epoch: {epoch}, train duration: {train_duration}, dev {self.task.dev_metric}: {dev_metrics[self.task.dev_metric]}, dev lsp acc: {dev_lsp_metrics["acc"]}, dev lsp macro f1: {dev_lsp_metrics["macro-f1"]}, test weighted-F1: {test_metrics["weighted-f1"]}, test macro-F1: {test_metrics["macro-f1"]}, test accuracy: {test_metrics["acc"]}')
            else:
                early_stopping_counter += 1
                self.result_writer.log(f'fold: {fold_num}, epoch: {epoch}, train duration: {train_duration}, dev {self.task.dev_metric}: {dev_metrics[self.task.dev_metric]}, dev lsp acc: {dev_lsp_metrics["acc"]}, dev lsp macro f1: {dev_lsp_metrics["macro-f1"]}')
