...
#         print('src: ', edges.src['z'])
#         print('dst: ', edges.dst['z'])
        wa = F.leaky_relu(self.attn_fc(z2))  # [edge_num, 1]
        return {'e': wa}
    ...
    def forward(self, g, h):
        ...
        snode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 1)
        wsedge_id = g.filter_edges(lambda edges: edges.data["dtype"] == 3)
...
class SWGATLayer(nn.Module):
    ...
    def forward(self, g, h):
        wnode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 2)
        snode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 1)
        swedge_id = g.filter_edges(lambda edges: edges.data["dtype"] == 2)
        z = self.fc(h)
        g.nodes[snode_id].data['z'] = z
...
class SSGATLayer(nn.Module):
    ...
    def forward(self, g, h):
        ...
        swedge_id = g.filter_edges(lambda edges: edges.data["dtype"] == 1)
        z = self.fc(h)
        g.nodes[snode_id].data['z'] = z
        g.apply_edges(self.edge_attention, edges=swedge_id)
        g.pull(snode_id, self.message_func, self.reduce_func)
...
#         print('dst: ', edges.dst['z'])
    ...
    def forward(self, g, h):
        wnode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 3)
        snode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 1)
        wsedge_id = g.filter_edges(lambda edges: edges.data["dtype"] == -3)
        # print("id in WSGATLayer")
        # print(wnode_id, snode_id, wsedge_id)
...
class SPGATLayer(nn.Module):
    ...
    def forward(self, g, h):
        wnode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 3)
        snode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 1)
        swedge_id = g.filter_edges(lambda edges: edges.data["dtype"] == 3)
        z = self.fc(h)
        g.nodes[snode_id].data['z'] = z
        g.apply_edges(self.edge_attention, edges=swedge_id)
        g.pull(wnode_id, self.message_func, self.reduce_func)
