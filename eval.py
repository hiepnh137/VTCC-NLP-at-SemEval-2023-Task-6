...
def calc_classification_metrics(y_true, y_predicted, labels):
    macro_precision, macro_recall, macro_f1, _ = precision_recall_fscore_support(y_true, y_predicted, average='macro', zero_division=0.0)
    micro_precision, micro_recall, micro_f1, _ = precision_recall_fscore_support(y_true, y_predicted, average='micro', zero_division=0.0)
    weighted_precision, weighted_recall, weighted_f1, _ = precision_recall_fscore_support(y_true, y_predicted, average='weighted', zero_division=0.0)
    per_label_precision, per_label_recall, per_label_f1, _ = precision_recall_fscore_support(y_true, y_predicted, average=None, labels=labels, zero_division=0.0)
...
def eval_model(model, eval_batches, device, task):
    ...
    doc_name_list = []
    true_lsp_labels = []
    predicted_lsp_labels = []
    with torch.no_grad():
        for batch in eval_batches:
            # move tensor to gpu
            tensor_dict_to_gpu(batch, device)
            batch['graph'][0] = batch['graph'][0].to(device)
            if batch["task"] != task.task_name:
                continue
            lsp_label = batch['lsp_label']
            copy_batch = batch.copy()
            copy_batch['lsp_label'] = None
            output, lsp_output = model(batch=copy_batch)
            ...
            true_lsp_labels_batch, predicted_lsp_labels_batch = batch["lsp_label"].view(-1), lsp_output["predicted_label"].view(-1)
            ...
            true_lsp_labels.extend(true_lsp_labels_batch)
            predicted_lsp_labels.extend(predicted_lsp_labels_batch)
    ...
    lsp_metrics, lsp_confusion, lsp_class_report = \
        calc_classification_metrics(y_true=true_labels, y_predicted=predicted_labels, labels=task.labels)
    return metrics, confusion,labels_dict, class_report, lsp_metrics, lsp_confusion, lsp_class_report
