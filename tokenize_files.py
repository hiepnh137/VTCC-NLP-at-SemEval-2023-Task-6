...
def write_in_hsln_format(input,hsln_format_txt_dirpath,tokenizer):
    ...
    for file in input:
        if len(file['annotations'][0]['result']) == 0 or len(file['annotations'][0]['result']) > 350:
            continue
        file_name=file['id']
...
def tokenize():
    ...
    os.makedirs('datasets/pubmed-20k-lsp/', exist_ok=True)
    write_in_hsln_format(train_json_format,'datasets/pubmed-20k-lsp/train_scibert.txt',tokenizer)
    write_in_hsln_format(dev_json_format, 'datasets/pubmed-20k-lsp/dev_scibert.txt', tokenizer)
    write_in_hsln_format(test_json_format, 'datasets/pubmed-20k-lsp/test_scibert.txt', tokenizer)
