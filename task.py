...
class Task:
    def __init__(self, task_name, labels, train_batch_size, num_fols, max_docs=-1,
                 dev_metric="micro-f1", portion_training_data=1.0,
    ...
    def load_graph(self):
        with open('graph/entity_graph/train_graph_1.pkl', 'rb') as f:
            train_graph = pkl.load(f)
        with open('graph/entity_graph/dev_graph_1.pkl', 'rb') as f:
            dev_graph = pkl.load(f)
        with open('graph/entity_graph/test_graph_1.pkl', 'rb') as f:
            test_graph = pkl.load(f)
        return train_graph, dev_graph, test_graph
    
    def load_cword_id(self):
        with open('cword/train_cword_id.pkl', 'rb') as f:
            train_cword_id = pkl.load(f)
        with open('cword/dev_cword_id.pkl', 'rb') as f:
            dev_cword_id = pkl.load(f)
        with open('cword/test_cword_id.pkl', 'rb') as f:
            test_cword_id = pkl.load(f)
        return train_cword_id, dev_cword_id, test_cword_id
    
    def load_lsp_embedding(self):
        with open(f'embedding/lsp_embedding/train.pkl', 'rb') as f:
            train_lsp_embedding = pkl.load(f)
        with open(f'embedding/lsp_embedding/dev.pkl', 'rb') as f:
            dev_lsp_embedding = pkl.load(f)
        with open(f'embedding/lsp_embedding/test.pkl', 'rb') as f:
            test_lsp_embedding = pkl.load(f)
            
        with open(f'embedding/sentence_embedding/train.pkl', 'rb') as f:
            train_sentence_embedding = pkl.load(f)
        with open(f'embedding/sentence_embedding/dev.pkl', 'rb') as f:
            dev_sentence_embedding = pkl.load(f)
        with open(f'embedding/sentence_embedding/test.pkl', 'rb') as f:
            test_sentence_embedding = pkl.load(f)
        
        train_embedding = self.merge_lsp_embedding(train_lsp_embedding, train_sentence_embedding)
        dev_embedding = self.merge_lsp_embedding(dev_lsp_embedding, dev_sentence_embedding)
        test_embedding = self.merge_lsp_embedding(test_lsp_embedding, test_sentence_embedding)

        return train_embedding, dev_embedding, test_embedding
    
    def load_paragraph(self):
        with open(f'graph/paragraph/train_paragraph.pkl', 'rb') as f:
            train_paragraph = pkl.load(f)
        with open(f'graph/paragraph/dev_paragraph.pkl', 'rb') as f:
            dev_paragraph = pkl.load(f)
        with open(f'graph/paragraph/test_paragraph.pkl', 'rb') as f:
            test_paragraph = pkl.load(f)
        return train_paragraph, dev_paragraph, test_paragraph
    
    def merge_lsp_embedding(self, lsp_embedding, sentence_embedding):
        merge_embedding = []
        for i in range(len(sentence_embedding)):
            doc_embedding = []
            for j in range(len(sentence_embedding[i])):
                doc_embedding.append(np.concatenate([lsp_embedding[i][j], sentence_embedding[i][j], lsp_embedding[i][j+1]], axis=0))
            merge_embedding.append(doc_embedding)
        return merge_embedding
    
    def load_lsp_tag(self):
        with open(f'embedding/tag/train.pkl', 'rb') as f:
            train_tag = pkl.load(f)
        with open(f'embedding/tag/dev.pkl', 'rb') as f:
            dev_tag = pkl.load(f)
        with open(f'embedding/tag/test.pkl', 'rb') as f:
            test_tag = pkl.load(f)
        return train_tag, dev_tag, test_tag
    ...
    def load_cword_span(self):
        with open(f'cword/1/train_cword.pkl', 'rb') as f:
            train_cword = pkl.load(f)
        with open(f'cword/1/dev_cword.pkl', 'rb') as f:
            dev_cword = pkl.load(f)
        with open(f'cword/1/test_cword.pkl', 'rb') as f:
            test_cword = pkl.load(f)
        train_cword_span = train_cword['spans']
        dev_cword_span = dev_cword['spans']
        test_cword_span = test_cword['spans']
        
#         flatten_train_cword_span = [self.flatten_span(span) for span in train_cword_span]
#         flatten_dev_cword_span = [self.flatten_span(span) for span in dev_cword_span]
#         flatten_test_cword_span = [self.flatten_span(span) for span in test_cword_span]
        return train_cword_span, dev_cword_span, test_cword_span
    
    def _load_train_dev_test_examples(self, file_suffix='scibert') -> Fold:

        log("Loading tokenized data...")
        train_graph, dev_graph, test_graph = self.load_graph()
#         train_cword_id, dev_cword_id, test_cword_id = None, None, None # self.load_cword_id()
        
        train_embedding, dev_embedding, test_embedding = self.load_lsp_embedding()
        train_tag, dev_tag, test_tag = self.load_lsp_tag()
#         train_paragraph, dev_paragraph, test_paragraph = self.load_paragraph()
        train_cword_span, dev_cword_span, test_cword_span = self.load_cword_span()
    
        train_examples = DocumentsDataset(train_graph, train_embedding, train_tag, os.path.join(self.data_dir, f"train_{file_suffix}.txt"), train_cword_span, max_docs=self.max_docs)
        dev_examples = DocumentsDataset(dev_graph, dev_embedding, dev_tag, os.path.join(self.data_dir, f"dev_{file_suffix}.txt"), dev_cword_span, max_docs=self.max_docs)
        test_examples = DocumentsDataset(test_graph, test_embedding, test_tag, os.path.join(self.data_dir, f"test_{file_suffix}.txt"), test_cword_span, max_docs=self.max_docs)
...
#         test_examples = dev_examples
    ...
    def get_folds(self):
        ...
        for train, dev, test in folds_examples:
            train_batches = self._get_batches(train)    #list of bucketing.BucketBatch
