...
#         flatten_test_cword_span = [self.flatten_span(span) for span in test_cword_span]
        return test_cword_span
    
    def _load_train_dev_test_examples(self, file_suffix='scibert') -> Fold:

        log("Loading tokenized data...")
        test_graph = self.load_graph()
        test_cword_id = [1 for i in range(len(test_graph))]
        test_embedding = self.load_lsp_embedding()
        test_tag = self.load_lsp_tag()
#         train_paragraph, dev_paragraph, test_paragraph = self.load_paragraph()
        test_cword_span = self.load_cword_span()
        test_examples = DocumentsDataset(test_cword_id, test_graph, test_embedding, test_tag, os.path.join(self.data_dir, f"test_{file_suffix}.txt"), test_cword_span, max_docs=self.max_docs)
...
#         test_examples = dev_examples
    ...
    def get_folds(self):
        ...
        for test in folds_examples:
            test_batches = self._get_batches(test)

            self.folds.append(Fold(test_batches))
